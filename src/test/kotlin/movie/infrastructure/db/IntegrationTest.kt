...
class IntegrationTest {
    ...
    fun `DB에서 영화를 조회하고 예매하고 결제까지 전체 흐름이 동작한다`() {
        ...
        val paymentResult = priceCalculator.calculate(
            reservations,
            Point(1000),
            CreditCard(),
        )
    ...
    fun `여러 영화를 예매하고 한꺼번에 결제할 수 있다`() {
        ...
        val updatedF1 = updatedMovies.findMovie("F1 더 무비")
            .getScreeningsByDate(date).findByNumber(1)
        assertThat(updatedF1.isSeatAvailable(
            Seat(SeatRow("C"), SeatColumn(1), SeatGrade.S),
        )).isFalse()

        val updatedToy = updatedMovies.findMovie("토이 스토리")
            .getScreeningsByDate(date).findByNumber(1)
        assertThat(updatedToy.isSeatAvailable(
            Seat(SeatRow("A"), SeatColumn(1), SeatGrade.B),
        )).isFalse()
    }
