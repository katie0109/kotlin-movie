...
class MovieApiControllerTest(
    ...
    fun `영화 목록 조회 응답에 영화와 상영 정보가 올바르게 포함된다`() {
        client
            ...
            .jsonPath("$.movies[0].movieId").exists()
            .jsonPath("$.movies[0].title").exists()
            .jsonPath("$.movies[0].runningTimeMinutes").exists()
            .jsonPath("$.movies[0].screenings").isArray()
            .jsonPath("$.movies[0].screenings[0].screeningId").exists()
            .jsonPath("$.movies[0].screenings[0].date").exists()
            .jsonPath("$.movies[0].screenings[0].startTime").exists()
            .jsonPath("$.movies[0].screenings[0].endTime").exists()
    }
}
